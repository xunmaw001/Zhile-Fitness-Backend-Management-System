    ...
    <fieldset class="search-container" style="text-align: center;margin-top: 50px;margin-bottom: 30px;">
      <form class="layui-form">
        ...
        <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
          ...
          <button v-if="isAuth('jianshenjiaolian','新增')" @click="jump('../jianshenjiaolian/add.jsp')" type="button" class="layui-btn btn-theme">
                  ...
                  <div class="gallery-item isotope-item print-media col-lg-3 col-sm-3 col-xs-6" v-for="(item,index) in dataList" v-bind:key="index">
                    <div class="inner-contents">
                      <figure style="text-align: center;">
                        <a class="gallery-btn swipebox" :href="'../jianshenjiaolian/detail.jsp?id='+item.id">查看更多</a>
                        <div class="media-container"></div>
                        <a :href="'../jianshenjiaolian/detail.jsp?id='+item.id">
                          <img :src="item.zhaopian?item.zhaopian.split(',')[0]:''" width="210" height="150" />
                      </figure>
                      <h5 class="item-title"><a :href="'../jianshenjiaolian/detail.jsp?id='+item.id">{{item.jiaolianxingming}}</a></h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#jiaolianxingming').val()) {
          param['jiaolianxingming'] = jquery('#jiaolianxingming').val() ? '%' + jquery('#jiaolianxingming').val() + '%' : '';
        }  
        if (jquery('#xingbie').val()) {
          param['xingbie'] = jquery('#xingbie').val() ? '%' + jquery('#xingbie').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('jianshenjiaolian/list', 'get', param, function(res) {
