  ...
  <div id="app">
    ...
    <fieldset class="search-container" style="text-align: center;margin-top: 50px;margin-bottom: 30px;">
      <form class="layui-form">
        ...
        <div class="layui-inline" style="margin-bottom: 10px;">
          <div class="layui-input-inline">
            <input type="text" name="jiaoliangonghao" id="jiaoliangonghao" placeholder="教练工号" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
          ...
          <button v-if="isAuth('kechenggoumai','新增')" @click="jump('../kechenggoumai/add.jsp')" type="button" class="layui-btn btn-theme">
                  ...
                  <div class="gallery-item isotope-item print-media col-lg-3 col-sm-3 col-xs-6" v-for="(item,index) in dataList" v-bind:key="index">
                    <div class="inner-contents">
                      <figure style="text-align: center;">
                        <a class="gallery-btn swipebox" :href="'../kechenggoumai/detail.jsp?id='+item.id">查看更多</a>
                        <div class="media-container"></div>
                        <a :href="'../kechenggoumai/detail.jsp?id='+item.id">
                          <img :src="item.tupian?item.tupian.split(',')[0]:''" width="210" height="150" />
                      </figure>
                      <h5 class="item-title"><a :href="'../kechenggoumai/detail.jsp?id='+item.id">{{item.kechengmingcheng}}</a></h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#jianshenleixing').val()) {
          param['jianshenleixing'] = jquery('#jianshenleixing').val() ? '%' + jquery('#jianshenleixing').val() + '%' : '';
        }  
        if (jquery('#jiaoliangonghao').val()) {
          param['jiaoliangonghao'] = jquery('#jiaoliangonghao').val() ? '%' + jquery('#jiaoliangonghao').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('kechenggoumai/list', 'get', param, function(res) {
