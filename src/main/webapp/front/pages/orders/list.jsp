      ...
      <form class="layui-form">
        <div class="layui-inline" style="margin-bottom: 10px;">
          <div class="layui-input-inline">
            <input type="text" name="orderid" id="orderid" placeholder="订单编号" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-bottom: 10px;">
          <div class="layui-input-inline">
        ...
        <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
          ...
          <button v-if="isAuth('orders','新增')" @click="jump('../orders/add.jsp')" type="button" class="layui-btn btn-theme">
                  ...
                  <div class="gallery-item isotope-item print-media col-lg-3 col-sm-3 col-xs-6" v-for="(item,index) in dataList" v-bind:key="index">
                    <div class="inner-contents">
                      <figure style="text-align: center;">
                        <a class="gallery-btn swipebox" :href="'../orders/detail.jsp?id='+item.id">查看更多</a>
                        <div class="media-container"></div>
                        <a :href="'../orders/detail.jsp?id='+item.id">
                      </figure>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#orderid').val()) {
          param['orderid'] = jquery('#orderid').val() ? '%' + jquery('#orderid').val() + '%' : '';
        }  
        if (jquery('#goodname').val()) {
          param['goodname'] = jquery('#goodname').val() ? '%' + jquery('#goodname').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('orders/list', 'get', param, function(res) {
