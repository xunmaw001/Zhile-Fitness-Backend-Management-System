      ...
      <form class="layui-form">
        <div class="layui-inline" style="margin-bottom: 10px;">
          <div class="layui-input-inline">
            <input type="text" name="wenzhangbiaoti" id="wenzhangbiaoti" placeholder="文章标题" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-bottom: 10px;">
          <div class="layui-input-inline">
        ...
        <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
          ...
          <button v-if="isAuth('jianshenzhishi','新增')" @click="jump('../jianshenzhishi/add.jsp')" type="button" class="layui-btn btn-theme">
                  ...
                  <div class="gallery-item isotope-item print-media col-lg-3 col-sm-3 col-xs-6" v-for="(item,index) in dataList" v-bind:key="index">
                    <div class="inner-contents">
                      <figure style="text-align: center;">
                        <a class="gallery-btn swipebox" :href="'../jianshenzhishi/detail.jsp?id='+item.id">查看更多</a>
                        <div class="media-container"></div>
                        <a :href="'../jianshenzhishi/detail.jsp?id='+item.id">
                          <img :src="item.tupianfengmian?item.tupianfengmian.split(',')[0]:''" width="210" height="150" />
                      </figure>
                      <h5 class="item-title"><a :href="'../jianshenzhishi/detail.jsp?id='+item.id">{{item.wenzhangbiaoti}}</a></h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#wenzhangbiaoti').val()) {
          param['wenzhangbiaoti'] = jquery('#wenzhangbiaoti').val() ? '%' + jquery('#wenzhangbiaoti').val() + '%' : '';
        }  
        if (jquery('#jianshenleixing').val()) {
          param['jianshenleixing'] = jquery('#jianshenleixing').val() ? '%' + jquery('#jianshenleixing').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('jianshenzhishi/list', 'get', param, function(res) {
