...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑健身商品</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>健身商品管理</span></li>
								<li class="breadcrumb-item active"><span>编辑健身商品</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">健身商品信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>商品种类</label>
										<input id="shangpinzhonglei" name="shangpinzhonglei" class="form-control" placeholder="商品种类">
									</div>
									<div class="form-group col-md-6">
										<label>商品图片</label>
										<div><img id="shangpintupianImg" src="" width="100" height="100">
										<div class="upload">选择文件<input name="file" type="file" id="shangpintupianupload" class="form-control-file"></div>
										<input name="shangpintupian" id="shangpintupian-input" type="hidden"></div>
									</div>
									<div class="form-group col-md-6">
										<label>商品价格</label>
										<input id="shangpinjiage" name="shangpinjiage" class="form-control" placeholder="商品价格">
									</div>
									<div class="form-group col-md-6">
										<label>规格</label>
									...
									<div class="form-group col-md-6">
										<label>作用</label>
										<textarea name="zuoyong" class="form-control" id="zuoyong" rows="3"></textarea>
									</div>
									<div class="form-group col-md-6">
										<label>价格</label>
	...
	<script>
		...
		var tableName = "jianshenshangpin";
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "jianshenshangpin/info/" + id,
		...
		function upload(){
			$('#shangpintupianupload').fileupload({
				...
				dataType: 'json',
					...
					document.getElementById('shangpintupian-input').setAttribute('value',baseUrl+"upload/"+data.result.file);
					if(document.getElementById('shangpintupianFileName') != null){
						document.getElementById('shangpintupianFileName').innerHTML = "上传成功!";
					}				
					$("#shangpintupianImg").attr("src",baseUrl+"upload/"+data.result.file);								
				}
			});
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("jianshenshangpin/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("jianshenshangpin/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("jianshenshangpin"+colName,"GET",{
				tableName: "jianshenshangpin",
				columnName: colName
			},(res)=>{
				if(res.code == 0){
		...
		function dataBind(){
			...
			$("#shangpinzhonglei").val(ruleForm.shangpinzhonglei);	
			$("#shangpintupian-input").val(ruleForm.shangpintupian);
			$("#shangpinjiage").val(ruleForm.shangpinjiage);	
			$("#guige").val(ruleForm.guige);	
			$("#baozhiqi").val(ruleForm.baozhiqi);	
			$("#zuoyong").val(ruleForm.zuoyong);
		...
		function showImg(){
			...
			$("#shangpintupianImg").attr("src",ruleForm.shangpintupian);
		}		
		//跨表
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.shangpinzhonglei != null && res.data.shangpinzhonglei != ''){

					$("#shangpinzhonglei").val(res.data.shangpinzhonglei);
					}
					if(res.data != null && res.data != '' && res.data.shangpintupian != null && res.data.shangpintupian != ''){

					$("#shangpintupianImg").attr("src",res.data.shangpintupian);
					$("#shangpintupian-input").val(res.data.shangpintupian);
					}
					if(res.data != null && res.data != '' && res.data.shangpinjiage != null && res.data.shangpinjiage != ''){

					$("#shangpinjiage").val(res.data.shangpinjiage);
					}
					...
					if(res.data != null && res.data != '' && res.data.zuoyong != null && res.data.zuoyong != ''){

					$("#zuoyong").val(res.data.zuoyong);
					}
