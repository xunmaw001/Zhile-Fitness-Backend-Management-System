...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">健身商品管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>健身商品列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">健身商品列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<input type="text" id="shangpinzhongleiSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('shangpinzhonglei')">商品种类<i id="shangpinzhongleiIcon" class="fa fa-sort"></i></th>

											<th>商品图片</th>
											<th onclick="sort('shangpinjiage')">商品价格<i id="shangpinjiageIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('guige')">规格<i id="guigeIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('baozhiqi')">保质期<i id="baozhiqiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#shangpinzhongleiSearch').val() != null && $('#shangpinzhongleiSearch').val() != ''){
				searchForm.shangpinzhonglei ="%" +  $('#shangpinzhongleiSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("jianshenshangpin/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				shangpinmingcheng : searchForm.shangpinmingcheng,	
				shangpinzhonglei : searchForm.shangpinzhonglei,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var shangpinzhongleiCell  = document.createElement('td');

						 shangpinzhongleiCell.innerHTML = item.shangpinzhonglei;


			row.appendChild(shangpinzhongleiCell);
			var shangpintupianCell  = document.createElement('td');

				                var shangpintupianImg = document.createElement('img');
			shangpintupianImg.setAttribute('src',item.shangpintupian);
			...
			var shangpinjiageCell  = document.createElement('td');

						 shangpinjiageCell.innerHTML = item.shangpinjiage;


			row.appendChild(shangpinjiageCell);
			...
			var baozhiqiCell  = document.createElement('td');

						 baozhiqiCell.innerHTML = item.baozhiqi;


			row.appendChild(baozhiqiCell);
			var zuoyongCell  = document.createElement('td');

						  if(item.zuoyong != null && item.zuoyong != "" && item.zuoyong.length >= 11){
			  	zuoyongCell.innerHTML = item.zuoyong.substring(0, 10) + "..."; 
			  }else{
			  	zuoyongCell.innerHTML = item.zuoyong;
			  }


			row.appendChild(zuoyongCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("jianshenshangpin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"jianshenshangpin");
